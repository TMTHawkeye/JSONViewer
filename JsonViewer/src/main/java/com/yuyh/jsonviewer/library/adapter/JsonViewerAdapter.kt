...
import com.yuyh.jsonviewer.library.adapter.JsonViewerAdapter.JsonItemViewHolder
...
class JsonViewerAdapter : BaseJsonViewerAdapter<JsonItemViewHolder?> {
    ...
    constructor(jsonStr: String?,contex:Activity) {
        ...
        if (`object` != null && `object` is JSONObject) {
            ...
            Toast.makeText(contex, "Error! File may have invalid format.", Toast.LENGTH_SHORT).show()
            contex.finish()
        }
    }
    ...
    override fun onBindViewHolder(holder: JsonItemViewHolder, position: Int) {
        val itemView: JsonItemView = holder.itemView as JsonItemView
        ...
        override fun onClick(view: View) {
            if (itemView.childCount == 1) { // 初始（折叠） --> 展开""
                ...
                for (i in 1 until itemView.childCount) {
                    itemView.getChildAt(i).visibility =
